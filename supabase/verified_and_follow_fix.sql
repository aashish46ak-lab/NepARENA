-- NepARENA: verified badge + organizer follow RLS helpers
-- Run in Supabase SQL editor.

-- 1) Profile verified flag (blue tick)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS is_verified boolean NOT NULL DEFAULT false;

-- Give founder verified badge
...
-- 2) Platform admin can toggle verification (service role / RPC)
...
-- 3) Secure organizer follow (fixes silent follow failures)
...
-- Optional RLS (adjust if your table already has policies)
...
-- 4) Platform admin hard-delete any DM message
...
-- Soft delete own message (if missing)
