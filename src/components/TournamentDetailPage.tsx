...
import {
  ...
  Loader2, ExternalLink, ArrowLeft, MoreHorizontal,
  Share2, MessageCircle, Flag, Gamepad2, CalendarDays,
} from "lucide-react";
...
export function TournamentDetailPage() {
  ...
  const knockoutMatches = useMemo(() => {
    const all = (data.matches ?? []) as unknown as import("@/lib/supabase").Match[];
    const ko = all.filter(
      (m) =>
    ...
    if (
      ...
      ["single_elimination", "double_elimination", "knockout"].includes(
        (tournament as Tournament).bracket_type ?? "",
      )
    ) {
      return all;
    }
    return ko.length ? ko : all;
  }, [data.matches, tournament]);
        ...
        <div className="mx-auto max-w-lg px-3 pt-4 sm:max-w-2xl sm:px-4">
          ...
          {tab === "bracket" && (
            <BracketTree
              ...
              players={data.participants as unknown as TournamentParticipant[]}
              tournamentName={name}
              tournamentLogo={(tournament as Tournament).logo_url}
              bannerUrl={(tournament as Tournament).banner_url}
