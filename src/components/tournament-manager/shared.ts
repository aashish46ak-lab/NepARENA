...
export async function recomputeStandings(
  ...
  if (rows.length > 0) {
    ...
    if (error) console.error("standings insert", error);
  }
...
export async function archiveTournamentToHistory(
  ...
  if (top3.length === 0) {
    warning = "No played results yet — history saved without podium.";
  ...
  await supabase.from("tournament_history").delete().eq("source_tournament_id", tournament.id);
  await supabase.from("tournament_history").delete().eq("tournament_name", tournament.name).eq("year", year);
  ...
  if (histErr && !/duplicate|unique|conflict/i.test(histErr.message || "")) {
    throw new Error("History save failed: " + histErr.message);
  }
  ...
  const load = useCallback(
    async (opts?: { silent?: boolean }) => {
      ...
      const [p, m, md, s, inv] = await Promise.all([
        ...
        supabase.from("matchdays").select("*").eq("tournament_id", tournamentId).order("sort_order"),
        supabase.from("tournament_standings").select("*").eq("tournament_id", tournamentId),
        supabase.from("tournament_invitations").select("*").eq("tournament_id", tournamentId).order("created_at", { ascending: false }),
      ]);
...
/** Rewrite legacy Group A·Matchday rows in DB → unified Matchday N. */
export async function migrateLegacyMatchdayNames(
  tournamentId: string,
  matchdays: Matchday[],
  _matches: Match[],
): Promise<boolean> {
  const dirty = matchdays.filter(
    (md) => /group/i.test(md.name) && /matchday/i.test(md.name),
  );
  if (dirty.length === 0) {
    // Still rename KO path suffixes if any
    let changed = false;
    for (const md of matchdays) {
      const norm = normalizeMatchdayLabel(md.name);
      if (norm !== md.name) {
        const { error } = await supabase.from("matchdays").update({ name: norm }).eq("id", md.id);
  ...
  const buckets = new Map<string, Matchday[]>();
  for (const md of dirty) {
    const norm = normalizeMatchdayLabel(md.name);
  ...
  let changed = false;
  for (const [norm, rows] of buckets) {
    rows.sort((a, b) => (a.sort_order ?? 999) - (b.sort_order ?? 999));
    const keeper = rows[0]!;
    const extras = rows.slice(1);
    if (keeper.name !== norm) {
      const { error } = await supabase.from("matchdays").update({ name: norm }).eq("id", keeper.id);
      if (!error) changed = true;
    }
    for (const extra of extras) {
      await supabase.from("matches").update({ matchday_id: keeper.id }).eq("matchday_id", extra.id);
      const { error } = await supabase.from("matchdays").delete().eq("id", extra.id);
      if (!error) changed = true;
    }
  }

  for (const md of matchdays) {
    if (dirty.some((d) => d.id === md.id)) continue;
    const norm = normalizeMatchdayLabel(md.name);
    if (norm !== md.name) {
      const { error } = await supabase.from("matchdays").update({ name: norm }).eq("id", md.id);
      if (!error) changed = true;
