...
async function publishAndNotify(
  ...
  const partIds = [...new Set(mdMatches.flatMap((m) => [m.home_id, m.away_id].filter(Boolean) as string[]))];
  ...
  const { data: parts } = await supabase
    ...
    .in("id", partIds)
    .eq("status", "approved");
  const userIds = [...new Set(((parts ?? []) as { user_id: string | null }[]).map((p) => p.user_id).filter(Boolean) as string[])];
...
export function FixturesTab({ tournament, data }: Props) {
  ...
  const groups = useMemo(() => {
    ...
    for (const m of data.matches) {
      ...
      else map.set(name, { id: md?.id ?? m.matchday_id, name, matches: [m] });
    }
    for (const md of data.matchdays) {
      if (!map.has(md.name)) map.set(md.name, { id: md.id, name: md.name, matches: [] });
    }
    return [...map.values()].sort((a, b) => {
      const oa = data.matchdays.find((d) => d.id === a.id)?.sort_order ?? 999;
      const ob = data.matchdays.find((d) => d.id === b.id)?.sort_order ?? 999;
      return oa - ob;
    });
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast.success(payload.length + " fixtures generated (" + bracketLabel(tournament.bracket_type ?? fmt.preset) + "). All matchdays are unpublished until you toggle Publish.");
  ...
  const onPublishToggle = async (on: boolean) => {
    if (!activeMdId || !activeName) { toast.error("Select a matchday first"); return; }
    setToggling(true);
    try {
      const n = await publishAndNotify(tournament, activeMdId, activeName, on, data.matches);
      toast.success(on ? activeName + " published to public & players notified" + (n ? " (" + n + ")" : "") : activeName + " unpublished — locked for public");
      data.reload();
    } catch (e) {
  ...
  return (
    <div className="space-y-4 pt-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {hasGroupStage(parseFormatConfig(tournament.format_config, tournament.bracket_type)) && (
          <Button type="button" variant="outline" disabled={busy || data.matches.length === 0} onClick={() => { setBusy(true); void seedKnockoutFromGroups(tournament, data).catch((e) => toast.error(e instanceof Error ? e.message : "Seed failed")).finally(() => setBusy(false)); }} title="Fill first knockout round from group tables (A1 vs B2)">
            Seed knockout from groups
          </Button>
        )}
        <Button type="button" variant="outline" disabled={busy || data.matches.length === 0} onClick={() => { setBusy(true); void advanceKnockoutWinners(tournament, data).catch((e) => toast.error(e instanceof Error ? e.message : "Advance failed")).finally(() => setBusy(false)); }} title="Push match winners into the next knockout round">
          Advance KO winners
        </Button>
      ...
      {data.matches.length === 0 && groups.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border/60 p-10 text-center text-sm text-muted-foreground">
          No fixtures yet. Click <strong>Generate fixtures</strong>. Matchdays stay unpublished until you turn Publish on.
        </div>
      ) : (
        <>
          <div className="flex items-center gap-1.5 max-w-[420px] mx-auto">
            ...
            <div className="flex flex-1 gap-2 overflow-x-auto snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden py-1">
              {groups.map((g) => {
                ...
                return (
                  <button key={g.name} ref={(el) => { if (el) tabRefs.current.set(g.name, el); else tabRefs.current.delete(g.name); }} type="button" onClick={() => selectMatchday(g.name)} className={cn("flex shrink-0 min-w-[100px] snap-center flex-col items-center rounded-xl border px-3 py-2 text-center transition", isActive ? "border-brand bg-brand/15" : "border-border/60 bg-secondary/30 hover:bg-secondary/50")}>
                    <div className={cn("text-xs font-semibold truncate w-full", isActive && "text-brand-glow")}>{g.name}</div>
                    <div className="text-[10px] text-muted-foreground mt-0.5">{String(played) + "/" + String(g.matches.length)}{pub ? " · Live" : " · Draft"}</div>
                  </button>
                );
              })}
            </div>
          ...
          {activeName && (
            <div className="glass w-full max-w-[420px] mx-auto rounded-2xl p-4 space-y-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                {isPublished ? "Visible on public tournament page + pending matches for players." : "Draft — public sees this matchday locked/blurred."}
              </p>
              <div className="space-y-2">
                {activeMatches.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">No matches in this matchday.</p>
                ) : (
