...
export function OrganizerPublicPage() {
  ...
  // Message tab: lock page scroll so only the chat thread scrolls
  useEffect(() => {
  ...
  return (
    <PageShell force="platform" hideChrome>
      <div className={cn("bg-[#0a0a0a]", tab === "message" ? "flex h-[100dvh] flex-col overflow-hidden" : "min-h-[100dvh] pb-24")}>
        <div className={cn("sticky top-0 z-40 border-b border-white/8 bg-[#0a0a0a]/95 backdrop-blur-md", tab === "message" && "shrink-0")}>
          <div ref={navRef} className="mx-auto flex max-w-lg gap-1.5 overflow-x-auto px-2 py-2 sm:max-w-2xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" style={{ WebkitOverflowScrolling: "touch" }}>
            {NAV.map((item) => {
              ...
              return (
                <button key={item.id} type="button" data-tab={item.id} onClick={() => onNav(item.id)} disabled={false}
                  className={cn(
              ...
              <div className="relative -mt-12 flex items-start gap-3 pl-4 sm:pl-6">
                ...
                <div className="flex min-w-0 flex-1 items-center gap-5 pt-2 text-xs text-neutral-400">
                  <Link
                    ...
                    className="rounded-xl px-1 py-0.5 transition hover:bg-white/[0.06] active:scale-[0.98]"
                  >
        ...
        <div className={cn(
          ...
          tab === "message"
            ? "flex min-h-0 flex-1 flex-col overflow-hidden px-3 pb-0 sm:px-4"
                ...
                {communityLinks.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {communityLinks.map((l) => {
                      ...
                      const looksLikeUrl = !raw || /^https?:\/\//i.test(raw) || /^www\./i.test(raw) || /[/.]/.test(raw);
                      ...
                      return (
                        <a
                          ...
                          title={display}
                          className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 transition hover:border-violet-400/40 hover:bg-white/[0.07]"
                        >
