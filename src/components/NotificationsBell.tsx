  ...
  const openNotif = async (n: NotifRow) => {
    ...
    try {
      ...
      if (u.pathname.startsWith("/messages")) {
        ...
        const withUser = u.searchParams.get("with") || undefined;
        void navigate({ to: "/messages", search: { c, with: withUser } as never });
        return;
      }
      if (u.pathname.startsWith("/posts/")) {
        const id = u.pathname.split("/").filter(Boolean)[1];
        if (id) { void navigate({ to: "/posts/$id", params: { id } }); return; }
      }
      ...
      if (u.pathname.startsWith("/tournaments/")) {
        const id = u.pathname.split("/").filter(Boolean)[1];
        if (id) { void navigate({ to: "/tournaments/$id", params: { id } }); return; }
      }
      if (u.pathname.startsWith("/o/")) {
        const slug = u.pathname.split("/").filter(Boolean)[1];
        if (slug) { void navigate({ to: "/o/$slug", params: { slug } }); return; }
      }
      void navigate({ to: (u.pathname + u.search) as "/" });
    } catch {
