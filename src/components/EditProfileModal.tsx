...
export function EditProfileModal({ open, onOpenChange, onSaved }: Props) {
  ...
  const save = async () => {
    ...
    if (form.favourite_player.trim()) social_links.favourite_player = form.favourite_player.trim();
    ...
    if (form.whatsapp.trim()) social_links.whatsapp = form.whatsapp.trim();
      ...
      <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden border-white/12 bg-[#0e0e10] p-0 sm:max-w-lg [&>button]:hidden">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                preview ? "bg-white/15 text-white" : "text-neutral-400 hover:bg-white/8 hover:text-white",
              )}
            >
          ...
          {loading || !form ? (
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">
              ...
              <div className="relative px-4 pb-5">
                ...
                <p className="text-lg font-semibold text-white">{form.full_name || form.username || "Display name"}</p>
                {form.username && <p className="text-sm text-neutral-400">@{form.username}</p>}
                {form.bio && <p className="mt-2 whitespace-pre-wrap text-sm text-neutral-300">{form.bio}</p>}
              </div>
            </div>
          ) : (
            <div className="space-y-5">
              <div>
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-neutral-500">Banner</p>
                <ImageUpload value={form.banner_url} onChange={(url) => set("banner_url", url)} folder="profiles" aspect="wide" label="" />
              </div>
              <div>
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-neutral-500">Profile photo</p>
                <ImageUpload value={form.avatar_url} onChange={(url) => set("avatar_url", url)} folder="profiles" aspect="square" label="" />
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-400">Display name</label>
                  <Input value={form.full_name} onChange={(e) => set("full_name", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="Your name" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-400">Username</label>
                  <Input value={form.username} onChange={(e) => set("username", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="unique_handle" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-neutral-400">Bio</label>
                <Textarea value={form.bio} onChange={(e) => set("bio", e.target.value)} rows={3} maxLength={280} className="resize-none border-white/10 bg-white/[0.04]" placeholder="Tell the community about yourself" />
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-400">Country</label>
                  <Input value={form.country} onChange={(e) => set("country", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="Nepal" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-400">Favorite club</label>
                  <Input value={form.favourite_club} onChange={(e) => set("favourite_club", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="e.g. Barcelona" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-400">Favorite national team</label>
                  <Input value={form.favourite_national_team} onChange={(e) => set("favourite_national_team", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="e.g. Brazil" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-400">Favorite player</label>
                  <Input value={form.favourite_player} onChange={(e) => set("favourite_player", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="e.g. Messi" />
                </div>
              </div>
              <div>
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-neutral-500">Social links</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  {(["facebook", "instagram", "whatsapp", "twitter", "tiktok", "youtube"] as const).map((key) => (
                    <Input
                      key={key}
                      value={form[key]}
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.tags} onChange={(e) => set("tags", e.target.value)} className="h-10 border-white/10 bg-white/[0.04]" placeholder="efootball, striker, nepal (comma separated)" />
              </div>
            </div>
          )}
        </div>
        ...
        {!loading && form && (
          <div className="shrink-0 border-t border-white/8 px-4 py-3">
            <Button className="w-full rounded-xl bg-white font-semibold text-black hover:bg-neutral-200" disabled={saving} onClick={() => void save()}>
