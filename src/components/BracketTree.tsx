...
/** Phone-first: auto-fit width so full bracket visible without horizontal scroll */
const CARD_W = 100;
const CARD_H = 38;
const COL_GAP = 10;
const ROUND_PAD_X = 4;
...
function layoutTree(rounds: RoundCol[]) {
  ...
  const totalHeight =
    Math.max(...last.map((c) => c + CARD_H / 2), n0 * unit) + 8;
    ...
    >
      {isFinal && (
        <div className="flex h-3 items-center justify-center gap-0.5 border-b border-amber-400/25 bg-amber-400/12">
          ...
          <span className="text-[6px] font-bold uppercase tracking-wider text-amber-200">
            Final
          </span>
        </div>
      )}
      <div
        className={cn(
          "flex flex-col",
          isFinal ? "h-[calc(100%-0.75rem)]" : "h-full",
        )}
      >
...
function Row({
  ...
  return (
    <div
      className={cn(
        "flex flex-1 items-center gap-0.5 px-1",
        win && "bg-emerald-500/15",
      )}
    >
      <Avatar className="h-4 w-4 shrink-0 rounded ring-1 ring-white/10">
      ...
      <span
        className={cn(
          "min-w-0 flex-1 truncate text-[9px] font-semibold leading-none",
          ph
      ...
      <span
        className={cn(
          "w-3.5 shrink-0 text-right text-[9px] font-bold tabular-nums",
          score == null
            ? "text-white/15"
            : win
              ? "text-emerald-300"
  ...
  return (
    <div className="mx-auto w-full max-w-[200px] overflow-hidden rounded-md border border-violet-400/30 bg-violet-500/10">
      <div className="flex items-center justify-center gap-1 border-b border-violet-400/20 py-0.5">
        ...
        <span className="text-[8px] font-bold uppercase tracking-wider text-violet-200">
          3rd Place
        </span>
      </div>
...
export function BracketTree({
  ...
  useLayoutEffect(() => {
    ...
    const apply = () => {
      ...
      if (w > 0) setFitScale(Math.min(1, w / treeWidth));
    };
    apply();
  ...
  return (
    ...
    >
      ...
      <div
        ref={viewportRef}
        className={cn(
          "relative z-[1] overflow-x-hidden overflow-y-auto",
          fullscreen ? "max-h-[calc(100dvh-140px)]" : "max-h-[min(70vh,560px)]",
      ...
      {thirdPlace.length > 0 && (
        <div className="relative z-[1] space-y-1.5 border-t border-white/10 px-2 py-2">
          {thirdPlace.map((m) => (
