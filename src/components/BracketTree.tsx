...
/* build-bust: 2026-08-21-bracket-v3 */
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex flex-col", isFinal ? "h-[calc(100%-0.75rem)]" : "h-full")}>
        <Row name={homeName} photo={hp} score={hs} win={homeWin} ph={!showHome} href={showHome ? userIdOf(players, m.home_id) : null} />
        <div className="border-t border-white/10" />
        <Row name={awayName} photo={ap} score={ascore} win={awayWin} ph={!showAway} href={showAway ? userIdOf(players, m.away_id) : null} />
      </div>
    </div>
  );
}
function Row({
  name, photo, score, win, ph, href,
}: {
  name: string; photo: string | null; score: number | null; win: boolean; ph?: boolean; href?: string | null;
}) {
  return (
    <div className={cn("flex flex-1 items-center gap-0.5 px-1", win && "bg-emerald-500/15")}>
      <Avatar className="h-5 w-5 shrink-0 rounded ring-1 ring-white/10">
        ...
        <AvatarFallback className={cn("rounded text-[7px] font-bold", ph ? "bg-sky-500/25 text-sky-200" : "bg-white/10")}>
          {ph ? "•" : name.slice(0, 2).toUpperCase()}
        </AvatarFallback>
      </Avatar>
      {href ? (
        <Link to="/members/$id" params={{ id: href }} className={cn("min-w-0 flex-1 truncate text-[10px] font-semibold leading-none hover:underline", ph ? "font-bold tracking-wide text-sky-300" : win ? "text-emerald-100" : "text-white/95")} onClick={(e) => e.stopPropagation()}>
          {name}
        </Link>
      ) : (
        <span className={cn("min-w-0 flex-1 truncate text-[10px] font-semibold leading-none", ph ? "font-bold tracking-wide text-sky-300" : win ? "text-emerald-100" : "text-white/95")}>
          {name}
        </span>
      )}
      <span className={cn("w-3.5 shrink-0 text-right text-[10px] font-bold tabular-nums", score == null ? "text-white/15" : win ? "text-emerald-300" : "text-white/65")}>
        {score != null ? score : "–"}
      </span>
    </div>
  );
...
export function BracketTree({
  matches, players, locked, tournamentName, tournamentLogo, organizerName, organizerLogo, bannerUrl: _bannerUrl, eventDate, allMatches, groupCount = 4,
}: {
  matches: Match[]; players: TournamentParticipant[]; locked?: boolean; tournamentName?: string; tournamentLogo?: string | null; organizerName?: string | null; organizerLogo?: string | null; bannerUrl?: string | null; eventDate?: string | null; allMatches?: Match[]; groupCount?: number;
}) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        fullscreen ? "fixed inset-0 z-[200] rounded-none" : "rounded-2xl shadow-[0_8px_40px_-12px_rgba(0,0,0,0.7)]",
        locked && "select-none",
      )}
    >
        ...
        <div className="flex w-full items-center justify-between gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <div className="min-w-0">
              {organizerName && (
                <p className="truncate text-[8px] font-semibold uppercase tracking-wider text-sky-300/85">{organizerName}</p>
              )}
              ...
              <p className="text-[8px] text-amber-200/80">
                Official Bracket · Live{dateLabel ? ` · ${dateLabel}` : ""}
              </p>
            </div>
          </div>
      ...
      {mainRounds.length === 0 ? (
        ...
        <div ref={viewportRef} className={cn("relative z-[1] overflow-x-auto overflow-y-auto overscroll-x-contain", fullscreen ? "max-h-[calc(100dvh-140px)]" : "max-h-[min(75vh,640px)]", locked && "pointer-events-none")}>
          ...
          <div className="relative mx-auto" style={{ width: Math.max(treeWidth * zoom, 0), height: Math.max((totalHeight + 36) * zoom, 120) }}>
            <div className="absolute left-0 top-0 origin-top-left" style={{ width: treeWidth, height: totalHeight + 36, transform: `scale(${zoom})`, transformOrigin: "top left", isolation: "isolate", willChange: "transform" }}>
