...
import {MoreVertical,Pencil,ImagePlus,Trash2,Settings2,Trophy,Radio} from "lucide-react";
...
toast.success("Tournament moved to history");
}
};
...
{t.status==="completed"?"Ended":t.status.replace("_"," ")}
</Badge>
</div>
...
<DropdownMenuItem onClick={()=>changeStatus(t,"completed")}>
...
aspect="wide"
onChange={async(url)=>{
await update(banner.id,{banner_url:url} as Partial<Tournament>);
...
<Select
value={values.status}
onValueChange={v=>set({status:v as Tournament["status"]})}
>
<SelectTrigger>
<SelectValue/>
</SelectTrigger>
...
<Input
type="datetime-local"
value={values.starts_at?values.starts_at.slice(0,16):""}
onChange={e=>set({
