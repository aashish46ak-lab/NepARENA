...
export function generateGroupFixtures(groups: string[][]): FixtureSpec[] {
  ...
  groups.forEach((g, gi) => {
    const specs = roundRobin(
      g, 1, (n) => `Group ${groupNames[gi]} · Matchday ${n}`, false,
...
function pathHintForKo(position: number, groupCount: number): string {
  const letters = "ABCDEFGH".slice(0, Math.min(groupCount, 26)).split("");
  if (letters.length < 2) return "";
  const pairIdx = position - 1;
  const a = letters[(pairIdx * 2) % letters.length] ?? "A";
  const b = letters[(pairIdx * 2 + 1) % letters.length] ?? "B";
  if (pairIdx % 2 === 0) return ` · ${a}1 vs ${b}2`;
  return ` · ${b}1 vs ${a}2`;
}
    ...
    groups.forEach((g, gi) => {
      for (let leg = 1; leg <= legs; leg++) {
        ...
        const specs = roundRobin(
          ...
          (n) => `Group ${gName} · Matchday ${n}${legs > 1 ? ` · L${leg}` : ""}`,
    ...
    if (entrants.length < 2) {
      ...
      for (let r = 1; r <= totalRounds; r++) {
        ...
        for (let p = 1; p <= matches; p++) {
          ...
          const hint = r === 1 && priorGroup ? pathHintForKo(p, groupCount) : "";
          for (let leg = 1; leg <= legs; leg++) {
            const base = legs > 1
...
export function generateFixtures(type: string, ids: string[]): FixtureSpec[] {
  ...
  switch (type) {
    ...
    case "group_only": {
      ...
      const out: FixtureSpec[] = [];
      groups.forEach((g, gi) => {
        out.push(...roundRobin(g, 1, (n) => `Group ${names[gi]} · Matchday ${n}`, false, { stage_type: "group", group_key: names[gi] }));
      });
      return out;
    }
...
/** Per-group table rows from played group matches (client-side). */
...
/**
 * Build home/away participant ids for first KO round from group tables.
 * Returns map position -> { home_id, away_id }.
 */
export function seedFirstKnockoutRound(
  ...
  for (let p = 1; p <= firstRoundMatchCount; p++) {
    ...
    if (pairIdx % 2 === 0) {
      result.set(p, {
        home_id: pick(a, 1),
        away_id: q >= 2 ? pick(b, 2) : pick(b, 1),
      });
    } else {
      result.set(p, {
        home_id: pick(b, 1),
        away_id: q >= 2 ? pick(a, 2) : pick(a, 1),
      });
    }
  }
  return result;
