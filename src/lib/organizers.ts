...
/** Permanently remove an organizer (hard delete preferred; soft-remove if FKs block).
 * Allowed DB statuses: pending | active | suspended | rejected — never "deleted".
 * After success: public URL 404s, memberships gone, dashboard empty for that org.
 */
export async function permanentlyRemoveOrganizer(
  organizerId: string,
  opts?: { slug?: string; name?: string },
) {
  const slug = opts?.slug || "org";
  const name = opts?.name || "Organizer";
  const goneSlug = `${slug.replace(/-gone-\d+$/, "")}-gone-${Date.now()}`.slice(0, 60);
  const goneName = `[removed] ${name}`.slice(0, 120);

  await supabase.from("organizer_members").delete().eq("organizer_id", organizerId);
  await supabase.from("organizer_followers").delete().eq("organizer_id", organizerId);

  for (const table of [
    "organizer_messages",
    "community_links",
    "organizer_invitations",
    "organizer_chat_members",
    "organizer_requests",
  ] as const) {
    try {
      await supabase.from(table).delete().eq("organizer_id", organizerId);
    } catch {
      /* ignore */
    }
  }
  ...
  try {
    await supabase.from("tournaments").update({ organizer_id: null }).eq("organizer_id", organizerId);
  } catch {
    /* ignore */
  }
  try {
    await supabase.from("posts").update({ organizer_id: null }).eq("organizer_id", organizerId);
  } catch {
    /* ignore */
  }
  try {
    await supabase.from("conversations").update({ organizer_id: null }).eq("organizer_id", organizerId);
  } catch {
    /* ignore */
  }

  const { error: delErr } = await supabase.from("organizers").delete().eq("id", organizerId);
  if (!delErr) {
    return { ok: true as const, mode: "hard" as const };
  }

  // Soft-remove: NEVER use status "deleted" (violates organizers_status_check)
  for (const status of ["rejected", "suspended"] as const) {
    ...
    if (!upErr) {
      await supabase.from("organizer_members").delete().eq("organizer_id", organizerId);
      return { ok: true as const, mode: "soft" as const, status };
    }
  }

  try {
    const { error: rpcErr } = await supabase.rpc("admin_set_organizer_status", {
    ...
    if (!rpcErr) {
      await supabase
        .from("organizers")
        .update({ slug: goneSlug, name: goneName, owner_user_id: null, is_verified: false } as never)
        .eq("id", organizerId);
      await supabase.from("organizer_members").delete().eq("organizer_id", organizerId);
      return { ok: true as const, mode: "soft" as const, status: "rejected" as const };
    }
  } catch {
    /* ignore */
  }
  ...
  return {
    ok: false as const,
    error: delErr.message || "Could not remove organizer (FK or RLS).",
  };
}
