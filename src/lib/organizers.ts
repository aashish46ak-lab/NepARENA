...
export async function listPublicOrganizers(): Promise<Organizer[]> {
  const { data, error } = await supabase
    ...
    .limit(80);
  if (error) {
    console.warn("listPublicOrganizers", error.message);
    return [];
  }
...
export async function loadPlatformAdminStats() {
  const [
    playersRes,
    tournamentsRes,
    liveRes,
  ...
  return {
    ...
    completedTournaments: doneRes.count ?? 0,
    organizers: organizersList.length,
    organizersList,
...
export async function acceptOrganizerInvite(params: { token: string; userId: string }) {
...
export async function listOrganizerMemberships(
  ...
  return rows
    .filter((r) => map.has(r.organizer_id))
    .map((r) => ({ ...r, organizer: map.get(r.organizer_id) }));
}
