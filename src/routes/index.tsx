...
const ABOUT_BLOCKS = [
  ...
  },
  {
    title: "Vision",
    icon: Eye,
    body: "Become the standard multi-organizer infrastructure for competitive eFootball across Nepal, from local cups to national seasons.",
  },
  {
    title: "About NepARENA",
    icon: Sparkles,
    body: "NepARENA is a platform, not a single organizer. We provide the technology layer so communities keep their identity while players share one account across organizers.",
  },
  {
  ...
  },
  {
    ...
    icon: CheckCircle2,
    body: "Multi-tenant dashboards, verified organizers, shared player identity, result verification, standings, and room for many leagues — without mixing brands or themes.",
  },
  {
    title: "Future goals",
    icon: Trophy,
    body: "National organizer network, better discovery, mobile-first PWA, and tools that scale from neighborhood cups to long competitive seasons.",
  },
];
        ...
        <div className="relative mx-auto max-w-3xl px-4 pb-10">
          ...
          <div className="relative mt-8">
            ...
            <div
              className={
                aboutOpen
                  ? "mt-5 space-y-5 overflow-hidden transition-all duration-500 ease-out"
                  : "relative mt-5 max-h-24 overflow-hidden transition-all duration-500 ease-out"
              }
            >
              {!aboutOpen && (
              ...
              <div className="space-y-5">
                {(aboutOpen ? restBlocks : restBlocks.slice(0, 1)).map((b) => (
                  <ProfileBlock key={b.title} title={b.title} icon={b.icon}>
                    {b.body}
                  </ProfileBlock>
                ))}
