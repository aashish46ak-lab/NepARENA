...
function MessagesPage() {
  ...
  const startRecording = async () => {
    if (!user || !activeId || recording || busy) return;
    try {
              ...
              {!activeThread ? (
                ...
                <>
                          ...
                          <div className={cn("max-w-[75%] rounded-2xl px-3 py-2 text-sm", mine ? "bg-sky-500 text-white" : "bg-white/10 text-neutral-100")}>
                            {shared ? (
                              ...
                              <div className="flex min-w-[160px] items-center gap-2">
                                ...
                                <span className="shrink-0 text-[10px] opacity-70">{m.body!.replace("__voice__:", "")}s</span>
                              </div>
                            ) : (
                              <>
                  ...
                  <div className="flex items-center gap-1.5 border-t border-white/10 p-2">
                    ...
                    />
                    <Button type="button" size="icon" variant="ghost" className="shrink-0" disabled={busy || recording} onClick={() => photoInputRef.current?.click()} aria-label="Send photo">
                    ...
                    {recording ? (
                      <div className="flex min-w-0 flex-1 items-center gap-2 rounded-full border border-rose-500/40 bg-rose-500/10 px-3 py-2">
                        ...
                        <span className="text-xs font-medium text-rose-200">Recording {recSeconds}s</span>
                        <button type="button" onClick={() => stopRecording()} className="ml-auto grid h-8 w-8 place-items-center rounded-full bg-rose-500 text-white" aria-label="Stop recording">
                      ...
                      <>
                        ...
                        {text.trim() ? (
                          <Button type="button" size="icon" disabled={busy} onClick={() => void send()} className="shrink-0 bg-sky-500 text-white hover:bg-sky-400">
                            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                          </Button>
                        ) : (
                          <Button type="button" size="icon" disabled={busy} onClick={() => void startRecording()} className="shrink-0 bg-sky-500 text-white hover:bg-sky-400" aria-label="Record voice">
