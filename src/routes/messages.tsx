  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const mine = orgs.filter((o) => ids.has(o.id) && (o as { owner_user_id?: string }).owner_user_id && (o as { owner_user_id?: string }).owner_user_id !== user.id);
        setFollowedOrgs(mine.length ? mine : orgs.filter((o) => (o as { owner_user_id?: string }).owner_user_id && (o as { owner_user_id?: string }).owner_user_id !== user.id).slice(0, 12));
      } catch { /* ignore */ }
    })();
  }, [user?.id]);
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          ).filter((o) => (o as { owner_user_id?: string }).owner_user_id && (o as { owner_user_id?: string }).owner_user_id !== user.id);
          for (const o of matched.slice(0, 10)) {
            out.push({
              ...
              avatar: o.logo_url,
              peerId: (o as { owner_user_id: string }).owner_user_id,
        ...
        <div className={cn("mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col overflow-hidden", mobileChat ? "px-0 pt-0" : "px-2 pt-2 sm:px-4")}>
          {!mobileChat && (
            <>
              ...
              {tab === "inbox" && chatHeads.length > 0 && (
                <div className="mb-2 flex gap-3 overflow-x-auto overscroll-x-contain pb-1 [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {chatHeads.map((t) => (
                    <button
                    ...
                    >
                      <Avatar className={cn("h-12 w-12", t.unread > 0 && "ring-2 ring-sky-400")}>
                      ...
                      {t.unread > 0 && (
                        <span className="absolute right-0 top-0 h-2.5 w-2.5 rounded-full bg-sky-500 ring-2 ring-[#0a0a0a]" />
                      )}
                    </button>
                  ))}
                </div>
