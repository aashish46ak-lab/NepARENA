...
function MessagesPage() {
  ...
  const inbox = useMemo(
    () =>
      threads.filter(
        (t) => t.status === "active" || (t.status === "request" && t.initiated_by === user?.id),
      ),
    [threads, user?.id],
  );
  ...
  const chatHeads = useMemo(
    () => inbox.filter((t) => t.status === "active").slice(0, 16),
    [inbox],
  );
    ...
    <PageShell force="platform" hideChrome>
      <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="mx-auto flex h-[calc(100vh-3.5rem)] max-w-5xl flex-col px-2 py-3 sm:px-4">
          <div className="mb-2 mt-1 flex gap-3 overflow-x-auto px-1 pb-1 pt-1">
            <button
              type="button"
              onClick={() => {
            ...
            {friendNotes.map((n) => (
              <div key={n.user_id} className="relative shrink-0 text-center">
                ...
                <p className="max-w-[3.5rem] truncate text-[9px] text-neutral-500">
                  {(n.name ?? "").split(" ")[0]}
                </p>
              </div>
            ))}
                  ...
                  >
                    ...
                    {tab === "requests" && (
                      <div className="mt-2 flex flex-wrap gap-2 pl-14">
                        <Button size="sm" className="h-7 bg-sky-500 px-3 text-xs text-white" onClick={() => void onAccept(t.conversation_id)}>
                          Accept
                        </Button>
                        <Button size="sm" variant="outline" className="h-7 border-white/15 px-3 text-xs" onClick={() => void onDecline(t.conversation_id)}>
                          Decline
                        </Button>
                        {!followingPeers[t.peer_id] && (
                          <Button size="sm" variant="ghost" className="h-7 px-3 text-xs text-sky-300" onClick={() => void onFollowBack(t.peer_id)}>
                            Follow back
                          </Button>
                        )}
                      </div>
                  ...
                  {activeThread.status === "request" && activeThread.initiated_by !== user?.id && (
                    <div className="flex flex-wrap items-center justify-center gap-2 border-b border-white/5 bg-white/[0.03] px-3 py-2">
                      ...
                      {!followingPeers[activeThread.peer_id] && (
                        <Button size="sm" variant="ghost" className="text-sky-300" onClick={() => void onFollowBack(activeThread.peer_id)}>Follow back</Button>
                      )}
                    </div>
                  )}
