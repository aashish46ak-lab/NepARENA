            ...
            <section className={cn("flex min-h-0 flex-1 flex-col", !mobileChat && "hidden md:flex")}>
              {!activeThread ? (
                ...
                <>
                    ...
                    {messages.map((m) => {
                      ...
                      return (
                        <div key={m.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                          <div className={cn("relative max-w-[75%] rounded-2xl px-3 py-2 text-sm", mine ? "bg-sky-500 text-white" : "bg-white/10 text-neutral-100")}>
                            {isVoice ? (
                  ...
                  <div className="flex shrink-0 items-center gap-1.5 border-t border-white/10 p-2">
                    ...
                    {recording ? (
                      <Button type="button" size="icon" className="shrink-0 bg-rose-500 text-white hover:bg-rose-400" onClick={stopVoice} aria-label="Stop recording">
                        <Square className="h-4 w-4" />
                      </Button>
                    ) : (
                      <Button type="button" size="icon" variant="ghost" className="shrink-0" disabled={busy} onClick={() => void startVoice()} aria-label="Record voice">
                        <Mic className="h-4 w-4 text-neutral-400" />
                      </Button>
                    )}
                    <Input value={text} onChange={(e) => setText(e.target.value)} placeholder={recording ? "Recording…" : "Message…"} className="border-white/10 bg-white/5" disabled={recording} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void send(); } }} />
                    <Button type="button" size="icon" disabled={busy || !text.trim() || recording} onClick={() => void send()} className="shrink-0 bg-sky-500 text-white hover:bg-sky-400">
