...
import { ArrowLeft, Loader2, Plus, Search, Send, Smile } from "lucide-react";
...
function MessagesPage() {
  ...
  const requests = useMemo(
    () =>
      threads.filter(
        (t) => t.status === "request" && t.initiated_by && t.initiated_by !== user?.id,
      ),
    [threads, user?.id],
  );
  ...
  const reloadThreads = async () => {
    ...
    setThreads(listAll);
    const activePeers = listAll
      .filter((t) => t.status === "active")
      .map((t) => t.peer_id)
      .slice(0, 24);
    const notes = await listFriendNotes(activePeers);
    setFriendNotes(notes);
    const reqPeers = listAll
      .filter((t) => t.status === "request" && t.initiated_by !== user.id)
      .map((t) => t.peer_id);
    if (reqPeers.length) {
      const map: Record<string, boolean> = {};
      await Promise.all(
        reqPeers.map(async (id) => {
          map[id] = await isFollowingUser(user.id, id);
        }),
      );
  ...
  useEffect(() => {
    ...
    void (async () => {
      setLoading(true);
      const note = await getMyNote(user.id);
      setMyNoteState(note);
      await reloadThreads();
      setLoading(false);
    })();
  ...
  useEffect(() => {
    ...
    void (async () => {
      const listM = await listDmMessages(activeId);
      setMsgs(listM);
  ...
  const onAccept = async (cid: string) => {
    const ok = await acceptDmRequest(cid);
    if (ok) {
  ...
  const onDecline = async (cid: string) => {
    const ok = await declineDmRequest(cid);
    if (ok) {
        ...
        <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0a0a0a]/95 backdrop-blur-md">
          <div className="mx-auto flex h-14 max-w-5xl items-center gap-3 px-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Link to="/" className="grid h-9 w-9 place-items-center rounded-full text-neutral-400 hover:bg-white/5">
        ...
        <div className="mx-auto flex h-[calc(100vh-3.5rem)] max-w-5xl flex-col px-2 py-3 sm:px-4">
          <div className="mb-3 flex gap-3 overflow-x-auto px-1 pb-1">
            ...
            >
              ...
              <p className="mt-1 max-w-[3.5rem] truncate text-[10px] text-sky-300">
                {myNote?.body || "Your note"}
              </p>
            </button>
            {friendNotes.map((n) => (
              <div key={n.user_id} className="relative shrink-0 text-center">
                ...
                <p className="mt-1 max-w-[3.5rem] truncate text-[10px] text-neutral-300">{n.body}</p>
              </div>
            ))}
            {inbox.slice(0, 12).map((t) => (
              <button
              ...
              <div className="h-full overflow-y-auto">
                ...
                {list.map((t) => (
                  <div
                    key={t.conversation_id}
                    className={cn(
                      "border-b border-white/5 px-3 py-3 transition hover:bg-white/[0.04]",
                  ...
                  >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-xs text-neutral-500">
                            {parseSharedPost(t.last_body) ? "Shared a post" : t.last_body || "Photo"}
                          </p>
                          {t.unread > 0 && (
                    ...
                    {tab === "requests" && (
                      <div className="mt-2 flex flex-wrap gap-2 pl-14">
                        <Button
                          size="sm"
                          className="h-7 bg-sky-500 px-3 text-xs text-white"
                          onClick={() => void onAccept(t.conversation_id)}
                        >
                          Accept
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-7 border-white/15 px-3 text-xs"
                          onClick={() => void onDecline(t.conversation_id)}
                        >
                          Decline
                        </Button>
                        {!followingPeers[t.peer_id] && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 px-3 text-xs text-sky-300"
                            onClick={() => void onFollowBack(t.peer_id)}
                          >
                            Follow back
                          </Button>
                        )}
                ...
                <>
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-neutral-500">
                          {activeThread.status === "request" ? "Message request" : "NepARENA chat"}
                        </p>
                      </div>
                    </Link>
                  </div>
                  ...
                  {activeThread.status === "request" && activeThread.initiated_by !== user?.id && (
                    <div className="flex flex-wrap items-center justify-center gap-2 border-b border-white/5 bg-white/[0.03] px-3 py-2">
                      <Button size="sm" className="bg-sky-500 text-white" onClick={() => void onAccept(activeThread.conversation_id)}>
                        Accept
                      </Button>
                      <Button size="sm" variant="outline" className="border-white/15" onClick={() => void onDecline(activeThread.conversation_id)}>
                        Decline
                      </Button>
                      {!followingPeers[activeThread.peer_id] && (
                        <Button size="sm" variant="ghost" className="text-sky-300" onClick={() => void onFollowBack(activeThread.peer_id)}>
                          Follow back
                        </Button>
                      )}
                    </div>
                        ...
                        <div key={m.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                          <div
                            className={cn(
                              ...
                              mine ? "rounded-br-md bg-sky-600 text-white" : "rounded-bl-md bg-white/10 text-neutral-100",
                            )}
                          >
                            {shared ? (
                              <a
                                href={shared.authorId ? `/members/${shared.authorId}` : "/feed"}
                                className="block overflow-hidden rounded-xl border border-white/15 bg-black/25 text-left transition hover:border-sky-400/40"
                              >
                                {shared.imageUrl && (
                                  <img src={shared.imageUrl} alt="" className="max-h-40 w-full object-cover" />
                                )}
                                <div className="p-2">
                                  ...
                                  {shared.text && (
                                    <p className="mt-0.5 line-clamp-3 text-xs opacity-90">{shared.text}</p>
                                  )}
                                </div>
                              </a>
                            ) : (
