...
import {
  ...
  sendDmMessage, setMyNote, type DmMessage, type DmThread, type UserNote,
} from "@/lib/dm";
...
import { ArrowLeft, ImagePlus, Loader2, Plus, Search, Send } from "lucide-react";
    ...
    <PageShell force="platform" hideChrome>
      <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="mx-auto flex h-[calc(100vh-3rem)] max-w-5xl flex-col px-2 py-3 sm:px-4">
          <div className="mb-2">
            <div className="relative">
              ...
              <Input
                ...
                className="h-10 rounded-xl border-white/10 bg-white/[0.05] pl-9 transition focus:border-sky-500/40"
              />
            </div>
          </div>

          <div className="mb-3 flex flex-nowrap items-start gap-3 overflow-x-auto px-1 pb-1 pt-2 scrollbar-thin">
            <button
              type="button"
              onClick={() => { setNoteDraft(myNote?.body ?? ""); setNoteOpen(true); }}
              className="relative shrink-0 pt-7 text-center transition active:scale-95"
            >
            ...
            {friendNotes.map((n) => {
              ...
              return (
                <button
                  key={`note-${n.user_id}`}
                  type="button"
                  onClick={() => { if (thread) openThread(thread.conversation_id); }}
                  className="relative shrink-0 pt-7 text-center transition active:scale-95"
                >
                  {n.body && (
                    <div className="absolute left-1/2 top-0 z-10 w-max max-w-[4.75rem] -translate-x-1/2">
            ...
            {chatHeads
              ...
              .map((t) => (
                <button
                  ...
                  className="relative shrink-0 pt-7 text-center transition active:scale-95"
                >
                  <Avatar className={cn("relative mt-0.5 h-14 w-14 ring-2 ring-offset-2 ring-offset-[#0a0a0a] transition", activeId === t.conversation_id ? "ring-sky-400" : "ring-white/10")}>
                    <AvatarImage src={t.peer_avatar ?? undefined} />
                    <AvatarFallback>{t.peer_name.slice(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  {t.unread > 0 && <span className="absolute right-0 top-7 h-3 w-3 rounded-full bg-sky-400 ring-2 ring-[#0a0a0a]" />}
          ...
          {noteOpen && (
            <div className="mb-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3 animate-in fade-in slide-in-from-top-1 duration-200">
              <p className="mb-1 text-xs font-semibold text-neutral-300">Note (max 20 chars · 24h)</p>
              <div className="flex flex-wrap gap-2">
                <Input value={noteDraft} onChange={(e) => setNoteDraft(e.target.value.slice(0, 20))} placeholder="What's up?" className="min-w-[8rem] flex-1 border-white/10 bg-black/30" maxLength={20} />
                <Button size="sm" className="bg-sky-500 text-white" onClick={async () => {
                  if (!user || !noteDraft.trim()) return;
                  const res = await setMyNote(user.id, noteDraft);
                  if (res.error) toast.error(res.error);
                  else { setMyNoteState(await getMyNote(user.id)); setNoteOpen(false); toast.success("Note saved"); }
                }}>Save</Button>
                {myNote && (
                  <Button size="sm" variant="ghost" className="text-rose-300" onClick={async () => {
                    if (user) { await deleteMyNote(user.id); setMyNoteState(null); setNoteDraft(""); setNoteOpen(false); toast.success("Note deleted"); }
                  }}>Delete</Button>
                )}
          ...
          <div className="mb-2 flex gap-2">
            <button type="button" onClick={() => setTab("inbox")} className={cn("rounded-full px-3 py-1 text-xs font-semibold transition", tab === "inbox" ? "bg-sky-500 text-white" : "bg-white/5 text-neutral-400 hover:bg-white/10")}>Inbox</button>
            <button type="button" onClick={() => setTab("requests")} className={cn("rounded-full px-3 py-1 text-xs font-semibold transition", tab === "requests" ? "bg-sky-500 text-white" : "bg-white/5 text-neutral-400 hover:bg-white/10")}>Requests{requests.length > 0 ? ` (${requests.length})` : ""}</button>
          </div>
