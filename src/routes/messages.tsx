...
function MessagesPage() {
  ...
  // Lock page scroll on Messages; full-screen chat hides bottom nav via data-mobile-chat
  useEffect(() => {
    ...
    return () => {
      ...
      document.body.removeAttribute("data-mobile-chat");
    };
  }, []);
  ...
  return (
    <PageShell force="platform" hideChrome>
      {/* Fixed viewport — body never scrolls; only inbox / chat list scroll */}
      <div className="fixed inset-0 z-10 flex flex-col overflow-hidden bg-[#0a0a0a] pb-[calc(4.5rem+env(safe-area-inset-bottom,0px))] pt-[env(safe-area-inset-top,0px)]">
        ...
        {/* Chat heads — horizontal scroll only; 4px top margin under header */}
        ...
        <div className="mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col overflow-hidden px-2 pt-2 sm:px-4">
          ...
          {/* Inbox boundary fixed — list scrolls inside only */}
          <div className="flex min-h-0 flex-1 overflow-hidden rounded-2xl border border-white/10 bg-black/40 shadow-inner">
            ...
            {/* Desktop: side panel. Mobile open chat: full-screen overlay covering nav + heads */}
            <section
              className={cn(
                "flex min-h-0 flex-1 flex-col bg-[#0a0a0a]",
                !mobileChat && "hidden md:flex",
                mobileChat &&
                  "fixed inset-0 z-[70] flex animate-in slide-in-from-right duration-200 md:static md:z-auto md:animate-none",
              )}
            >
              {!activeThread ? (
                ...
                <>
                  <div className="flex shrink-0 items-center gap-1 border-b border-white/8 bg-[#0a0a0a] px-1.5 py-2.5 pt-[max(0.5rem,env(safe-area-inset-top,0px))] backdrop-blur-md md:px-2 md:pt-2.5">
                    <Button
                      type="button"
                      size="icon"
                      variant="ghost"
                    ...
                    <Link to="/members/$id" params={{ id: activeThread.peer_id }} className="flex min-w-0 flex-1 items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-neutral-500">
                          {activeThread.is_group ? "Group chat" : activeThread.status === "request" ? "Message request" : "Active"}
                        </p>
                      </div>
                    </Link>
                  </div>
