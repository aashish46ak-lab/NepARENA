...
function OrganizerPublicPage() {
  ...
  const share = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title, url, text: `Join ${title} on NepARENA` });
      } else {
    ...
    return (
      <PageShell force="organizer">
        <div className="py-20 text-center text-muted-foreground">
          Organizer not found
          <div className="mt-4">
            <Link to="/" className="text-brand">Back to NepARENA</Link>
          </div>
        </div>
      </PageShell>
    );
        ...
        <div className="relative">
          ...
          <div className="relative mx-auto max-w-3xl px-4 pb-6">
            <div className="-mt-14 flex flex-col items-start gap-3 sm:-mt-16">
              {logo ? (
                <img src={logo} alt={organizer.name} className="h-24 w-24 rounded-2xl object-cover shadow-xl ring-4 ring-background sm:h-28 sm:w-28" />
              ) : (
              ...
              <div className="flex w-full flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl" style={{ textShadow: theme.nameShadow }}>
                      {organizer.name}
                    </h1>
                    {organizer.is_verified ? (
                      <Badge className="bg-sky-500/25 text-sky-200 hover:bg-sky-500/25">Verified</Badge>
                    ) : (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    ...
                    style={following ? undefined : { background: theme.cover, color: "#0a0a0a" }}
                    disabled={followBusy}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-1 text-sm text-muted-foreground">
              ...
              <Link
                to="/feed"
                className="inline-flex items-center gap-1.5 rounded-full border border-sky-500/30 bg-sky-500/10 px-2.5 py-0.5 text-xs font-medium text-sky-200 hover:bg-sky-500/20"
              >
                Feed
              </Link>
            </div>
        ...
        <div className="mx-auto max-w-3xl space-y-6 px-4 pb-14 pt-4">
                      ...
                      <div className="space-y-3 p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {st === "approved" || st === "registered" ? (
                            ...
                            <Button size="sm" style={{ background: theme.cover, color: "#0a0a0a" }} onClick={() => void requestJoin(t.id)}>
                              Request to join
                            </Button>
                          ) : (
          ...
          {communityLinks.length > 0 && (
            <section className="pt-2">
              <h2 className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Community links</h2>
              <div className="flex flex-wrap gap-2">
