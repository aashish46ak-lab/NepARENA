...
function OrganizerPublicPage() {
  ...
  const { data: isStaff = false } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (data) return true;
      const owner =
        (organizer as { owner_id?: string | null; owner_user_id?: string | null } | null)
          ?.owner_id ||
        (organizer as { owner_user_id?: string | null } | null)?.owner_user_id;
  ...
  const live = useMemo(
    () => tournaments.filter((t) => ["live", "ongoing", "check_in"].includes(String(t.status))),
    [tournaments],
  );
  const upcoming = useMemo(
    () =>
      tournaments.filter((t) =>
        ["upcoming", "registration_open", "registration_closed", "draft"].includes(
          String(t.status),
        ),
      ),
    [tournaments],
  ...
  const completed = useMemo(
    () =>
      tournaments.filter((t) => ["completed", "archived"].includes(String(t.status))),
    [tournaments],
  );
  ...
  const ownerId =
    (organizer as { owner_id?: string | null; owner_user_id?: string | null } | null)?.owner_id ||
    (organizer as { owner_user_id?: string | null } | null)?.owner_user_id ||
  ...
  const onNav = (id: TabId | "message") => {
    ...
    requestAnimationFrame(() => {
      ...
      if (!root) return;
      const active = root.querySelector<HTMLElement>(`[data-tab="${id}"]`);
      ...
      <PageShell force="platform" hideChrome>
        <div className="mx-auto max-w-md px-4 py-20 text-center">
          ...
          <Button asChild className="mt-4">
            <Link to="/organizers">Browse organizers</Link>
          </Button>
        </div>
      </PageShell>
    );
  ...
  const banner =
    (organizer as { cover_url?: string | null }).cover_url ||
    (organizer as { banner_url?: string | null }).banner_url ||
    null;
          ...
          >
            {NAV.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("h-4 w-4", active && "text-sky-400")} />
        ...
        <div className="relative">
          ...
          <div className="mx-auto max-w-lg px-3 sm:max-w-2xl sm:px-4">
              ...
              <div className="mb-0.5 min-w-0 flex-1 pb-0.5">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {primaryGame && (
                      <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-400/90">
                  ...
                  <button
                    ...
                    type="button"
                    onClick={openMenu}
                    className="shrink-0 rounded-full border border-white/12 bg-white/[0.04] p-2 text-neutral-300 hover:bg-white/[0.08]"
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-neutral-400">
              ...
              <span>
                <strong className="tabular-nums text-white">{postCount}</strong> posts
              </span>
              {tournaments.length > 0 && (
                <span className="inline-flex items-center gap-1">
                  <Trophy className="h-3 w-3 text-amber-400" />
                  <strong className="tabular-nums text-white">{tournaments.length}</strong> tournaments
                </span>
              )}
            </div>
          </div>
            ...
            <section className="space-y-3">
              ...
              {live.length === 0 ? (
                <Empty
                  text={
                    upcoming[0]
                      ? `Nothing live — next up: ${upcoming[0].name}`
                  ...
                  <div className="space-y-2">
                    {upcoming.slice(0, 6).map((t) => (
                      <TourneyRow
                        key={t.id}
                        t={t}
                        badge={String(t.status).replace(/_/g, " ")}
                ...
                <div className="space-y-2">
                  {completed.map((t) => (
                    <TourneyRow
                      ...
                      t={t}
                      badge="Completed"
                      badgeClass="bg-neutral-500/20 text-neutral-300"
                    />
                  ))}
                </div>
              )}
            ...
            <section className="space-y-4">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="min-w-0">
                  ...
                  {primaryGame && (
                    <p className="mt-0.5 text-[10px] font-semibold uppercase text-sky-400">
                      {String(primaryGame).replace(/_/g, " ")}
                    </p>
                  )}
                </div>
                  ...
                  <div className="space-y-2">
                    {team.map((m) => (
                      <Link
                        ...
                        to="/members/$id"
                        params={{ id: m.user_id }}
                        className="flex items-center gap-2.5 rounded-xl border border-white/8 bg-white/[0.02] px-3 py-2 transition hover:bg-white/[0.05]"
                      >
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={m.avatar_url ?? undefined} />
                          <AvatarFallback>
                            {(m.full_name || m.username || "?").slice(0, 1)}
                          </AvatarFallback>
                        </Avatar>
                ...
                <div className="grid gap-2">
                  {communityLinks.map((l) => (
                    <a
                      ...
                      className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-3.5 py-3 transition hover:border-violet-400/30 hover:bg-white/[0.06]"
                    >
            ...
            <>
              ...
              >
                {isStaff && (
                  <button
                    ...
                    onClick={() => {
                      setMenuOpen(false);
                      try {
                        localStorage.setItem("neparena-active-organizer-slug", organizer.slug);
                      } catch {
                        /* ignore */
                      }
                ...
                <button
                  ...
                  onClick={() => {
                    setMenuOpen(false);
                    toast.message("Thanks — report received for review");
                  }}
                >
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-[11px] text-neutral-500">
          {t.game ? `${t.game} · ` : ""}
          {t.starts_at ? new Date(t.starts_at).toLocaleDateString() : "Date TBA"}
        </p>
      </div>
      <span
        className={cn(
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center px-3 py-2.5">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="shrink-0 rounded-full bg-rose-500/25 px-2 py-0.5 text-[10px] font-bold uppercase text-rose-200">
            LIVE
          </span>
        </div>
        ...
        {(t.participants_count != null || t.max_players != null) && (
          <p className="mt-1 text-[11px] text-neutral-500">
            ...
            {t.max_players != null ? ` / ${t.max_players}` : ""} players
          </p>
        )}
