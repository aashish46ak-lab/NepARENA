...
function OrganizerPublicPage() {
  ...
  const toggleFollow = async () => {
    ...
    try {
      if (wasFollowing) {
        const res = await unfollowOrganizer(organizer.id, user.id);
        if ((res as { error?: { message?: string } | null })?.error) {
          throw new Error((res as { error: { message: string } }).error.message || "Unfollow failed");
        }
        toast.success("Unfollowed");
      } else {
        const res = await followOrganizer(organizer.id, user.id);
        if ((res as { error?: { message?: string } | null })?.error) {
          throw new Error((res as { error: { message: string } }).error.message || "Follow failed");
        }
    ...
    <PageShell force="organizer" hideChrome>
      <div className="min-h-screen pb-10" style={{ backgroundImage: theme.pageBg }}>
        ...
        <div className="mx-auto max-w-3xl space-y-8 px-4 pt-2">
          ...
          <footer className="mt-4 border-t border-white/10 pt-6 text-center text-xs text-neutral-500">
