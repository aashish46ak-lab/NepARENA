...
/**
 * BINARY ISOLATION LOG
 * Step 1 PWA/SW      → kept as unregister-only (safe)
 * Step 2 Splash      → RESTORED with /neparena-logo.png (was not the blank cause)
 * Step 3 Auth        → still ON (AuthProvider)
 * Step 4 RoleRedirect → DISABLED this commit ← test blank screen
 * Step 5 Theme       → none
 * Step 6 API         → ON
 * Step 7 Animations  → ON
 */
      ...
      <head>
        ...
        <style
          dangerouslySetInnerHTML={{
            __html:
              "html,body{margin:0;min-height:100%;background:#050b16;color:#e8eefc}" +
              BOOT_SPLASH_CSS,
          }}
        />
        <script
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){try{if("serviceWorker"in navigator){navigator.serviceWorker.getRegistrations().then(function(r){r.forEach(function(x){x.unregister()})})}if("caches"in window){caches.keys().then(function(k){k.forEach(function(n){caches.delete(n)})})}}catch(e){}function killSplash(){var el=document.getElementById("neparena-boot-splash");if(!el||el.classList.contains("out"))return;el.classList.add("out");setTimeout(function(){try{el.remove()}catch(e){}},600)}setTimeout(killSplash,1700);})();`,
          }}
        />
      </head>
...
function RootComponent() {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <AuthProvider>
        <ClientErrorBoundary>
          {/* Step 4 isolation: RoleRedirect DISABLED */}
          <Outlet />
