 ...
 * Step 1 PWA/SW     → DISABLED (unregister only)
 * Step 2 Splash     → DISABLED (this commit) ← test blank screen now
 * Step 3 Auth       → still ON
 * Step 4 RoleRedirect → still ON
 * Step 5 Theme      → none (CSS only)
 * Step 6 API calls  → still ON
 * Step 7 Animations → still ON (page UI only)
 */
  ...
  return (
    <html lang="en" suppressHydrationWarning>
      <head>
        ...
        />
        {/* Step 1 isolation: always unregister SW */}
        <script
          dangerouslySetInnerHTML={{
      ...
      <body className="bg-[#050b16] text-foreground antialiased" suppressHydrationWarning>
        {/* Step 2 isolation: NO boot splash */}
        {children}
        <Scripts />
      </body>
    </html>
