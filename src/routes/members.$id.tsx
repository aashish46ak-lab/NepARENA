...
function MemberProfilePage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { profile, parts: partList, tours, achievements, followingOrgs, followerCount, followingCount, streak };
    },
  });
  ...
  const { profile, parts, tours, achievements, followingOrgs, followerCount, followingCount, streak } = data;
    ...
    <PageShell force="platform" hideChrome>
      <div className="mx-auto max-w-lg pb-24">
        <div className="relative">
          <div className="relative h-36 overflow-hidden bg-gradient-to-br from-sky-900 via-slate-900 to-violet-950 sm:h-44">
            {banner ? <img src={banner} alt="" className="absolute inset-0 h-full w-full object-cover" /> : null}
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
          </div>
          <div className="relative px-4">
            <div className="flex items-end justify-between">
              <div className="-mt-12 h-24 w-24 overflow-hidden rounded-full ring-4 ring-[#0a0a0a]">
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-11 z-50 min-w-[200px] overflow-hidden rounded-2xl border border-white/12 bg-[#141416]/98 py-1.5 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150">
                        ...
                        {(isAdmin || isSuperAdmin) && (
                          <Link to="/admin" className="flex w-full items-center gap-3 px-3.5 py-2.5 text-left text-sm text-neutral-100 hover:bg-white/8" onClick={() => setMenuOpen(false)}>
                            <LayoutDashboard className="h-4 w-4 text-neutral-400" /> Admin
                          </Link>
                        )}
                        <button
                          type="button"
        ...
        {achievements.length > 0 && (
          <div className="mt-6 px-4">
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Achievements</h2>
        ...
        {followingOrgs.length > 0 && (
          <div className="mt-6 px-4">
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Following organizers</h2>
