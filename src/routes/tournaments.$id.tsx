...
function TournamentDetailPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "matches", filter: "tournament_id=eq." + id }, invalidate)
      .on("postgres_changes", { event: "*", schema: "public", table: "matchdays", filter: "tournament_id=eq." + id }, invalidate)
      .on("postgres_changes", { event: "*", schema: "public", table: "tournament_participants", filter: "tournament_id=eq." + id }, invalidate)
      .on("postgres_changes", { event: "*", schema: "public", table: "tournament_standings", filter: "tournament_id=eq." + id }, invalidate)
      .subscribe();
  ...
  return (
    <PageShell>
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 overflow-x-hidden">
        <div className="glass overflow-hidden rounded-3xl min-w-0">
          <div className="flex flex-col sm:flex-row gap-4 p-4 sm:p-6 min-w-0">
            ...
            <div className="flex-1 min-w-0 space-y-2 text-center sm:text-left">
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold leading-snug line-clamp-2 break-words">{tournament.name}</h1>
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5">
                <Badge className="bg-brand/25 text-brand-glow capitalize">{tournament.status.replace(/_/g, " ")}</Badge>
        ...
        <MyMatches tournament={tournament} matches={matches} matchdays={matchdays} allParticipants={allParticipants} players={players} />
        ...
        <div className="glass flex flex-wrap gap-2 rounded-2xl p-3">
          {(() => {
            ...
            return [
              ...
              ...(showStandings ? [{ id: "standings", label: "Standings", icon: Table2 }] : []),
              { id: "fixtures", label: "Fixtures", icon: List },
              ...(hasGroups ? [{ id: "groups", label: "Group Stage", icon: Users }] : []),
              ...(knockout ? [{ id: "bracket", label: "Bracket", icon: GitBranch }] : []),
              { id: "rules", label: "Rules", icon: FileText },
              { id: "report", label: "Report", icon: ShieldAlert },
            ];
          })().map((t) => (
            <button key={t.id} type="button" onClick={() => setTab(t.id)}
              className={cn("flex items-center gap-2 rounded-lg px-4 py-2 text-sm transition", tab === t.id ? "bg-primary text-primary-foreground" : "hover:bg-accent")}>
        ...
        <div className="glass rounded-2xl p-4 sm:p-6 min-w-0">
          {tab === "overview" && (
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              ...
              <Info icon={<Calendar className="h-5 w-5 text-brand-glow" />} title="Starts" value={tournament.starts_at ? new Date(tournament.starts_at).toLocaleDateString() : "—"} />
              <Info icon={<Award className="h-5 w-5 text-brand-glow" />} title="Prize Pool" value={tournament.prize_pool || "—"} />
              <Info icon={<List className="h-5 w-5 text-brand-glow" />} title="Matches" value={String(matches.filter((m) => m.played).length) + " / " + String(matches.length)} />
              <Info icon={<Banknote className="h-5 w-5 text-brand-glow" />} title="Entry Fee" value={Number(tournament.registration_fee ?? 0) > 0 ? "NPR " + Number(tournament.registration_fee).toLocaleString() : "Free"} />
              <Info icon={<Shuffle className="h-5 w-5 text-brand-glow" />} title="Format" value={bracketLabel(tournament.bracket_type)} />
            </div>
          )}

          {tab === "groups" && (<PublicFixtures matches={matches} matchdays={matchdays} players={players} nameFilter={(n) => n.startsWith("Group ")} />)}
          {tab === "bracket" && (<BracketTab matches={matches} matchdays={matchdays} players={players} bracketType={tournament.bracket_type ?? "single_elim"} />)}
                            ...
                            <td className="p-3">
                              <div className="flex items-center gap-2.5 min-w-0">
                                <Avatar className="h-8 w-8 shrink-0">
                                  ...
                                  <AvatarFallback className="bg-secondary text-[10px]">{label.slice(0, 2).toUpperCase()}</AvatarFallback>
                                </Avatar>
          ...
          {tab === "fixtures" && (<PublicFixtures matches={matches} matchdays={matchdays} players={players} />)}

          {tab === "rules" && (
            <div>
              <h2 className="mb-4 text-xl font-bold">Tournament Rules</h2>
              {tournament.rules_text ? (
                <p className="whitespace-pre-line text-sm leading-relaxed text-muted-foreground">{tournament.rules_text}</p>
              ) : tournament.rules_url ? (
                <Button asChild variant="outline"><a href={tournament.rules_url} target="_blank" rel="noreferrer"><ExternalLink className="mr-2 h-4 w-4" /> View rules document</a></Button>
              ) : (
                <p className="py-6 text-center text-sm text-muted-foreground">Rules for this tournament haven't been published yet.</p>
              )}
            </div>
          )}
...
function PublicFixtures({ matches, matchdays, players, nameFilter }: {
  matches: Match[]; matchdays: Matchday[]; players: TournamentParticipant[]; nameFilter?: (name: string) => boolean;
}) {
  const groups = useMemo(() => {
    type G = { id: string | null; name: string; published: boolean; matches: Match[]; sort: number };
    const byId = new Map<string, G>();
    const sortedMd = [...matchdays].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
    for (const md of sortedMd) {
      byId.set(md.id, { id: md.id, name: md.name, published: md.is_published === true, matches: [], sort: md.sort_order ?? 0 });
    }
    for (const m of matches) {
      if (m.matchday_id && byId.has(m.matchday_id)) {
        ...
        if (!byId.has(key)) {
          byId.set(key, { id: m.matchday_id, name, published: false, matches: [], sort: 999 + m.round });
        }
        byId.get(key)!.matches.push(m);
      }
    }
    return [...byId.values()].filter((g) => !nameFilter || nameFilter(g.name)).sort((a, b) => a.sort - b.sort);
  ...
  if (groups.length === 0) {
    return <p className="py-6 text-center text-sm text-muted-foreground">Fixtures haven't been generated yet.</p>;
  }
    ...
    <div className="space-y-4">
      <div className="mx-auto flex w-full max-w-[340px] gap-2 overflow-x-auto snap-x snap-mandatory scroll-smooth py-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {groups.map((g) => {
          ...
          return (
            <button key={g.name} ref={(el) => { if (el) tabRefs.current.set(g.name, el); else tabRefs.current.delete(g.name); }}
              type="button" onClick={() => selectMatchday(g.name)}
              className={cn("flex shrink-0 min-w-[100px] snap-center flex-col items-center rounded-xl border px-3 py-2 text-center transition",
                isActive ? "border-brand bg-brand/15" : "border-border/60 bg-secondary/30 hover:bg-secondary/50")}>
              <div className={cn("text-xs font-semibold truncate w-full", isActive && "text-brand-glow")}>{g.name}</div>
              <div className="text-[10px] text-muted-foreground mt-0.5">
                {g.published ? String(played) + "/" + String(g.matches.length) + " Completed" : "Locked"}
              </div>
            </button>
          );
        })}
      ...
      {activeName && (
        <div className="relative glass mx-auto w-full max-w-[340px] min-h-[180px] overflow-hidden rounded-2xl p-4">
          <div className={cn("space-y-2", !isPublished && "pointer-events-none select-none blur-sm")} aria-hidden={!isPublished}>
            {activeMatches.length === 0 ? (
              ...
              activeMatches.map((m) => {
                ...
                const score =
                  isPublished && m.played && m.home_score != null && m.away_score != null
                    ? String(m.home_score) + " - " + String(m.away_score)
                    : "";
                return (
                  <div key={m.id} className="flex items-center gap-2 rounded-xl border border-border/60 px-3 py-2.5">
                    <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
                      <span className="max-w-[120px] truncate text-right text-sm font-semibold">{home}</span>
                      <Avatar className="h-8 w-8 shrink-0">
                    ...
                    <div className="w-14 shrink-0 text-center text-sm font-bold text-brand-glow">{score || "\u00A0"}</div>
          ...
          {!isPublished && (
            <div className="absolute inset-0 z-10 grid place-items-center bg-background/55 backdrop-blur-[2px]">
              <div className="flex flex-col items-center gap-1.5 px-4 text-center">
                ...
                <p className="max-w-[220px] text-xs text-muted-foreground">This matchday will be revealed when the admin publishes it.</p>
              </div>
            </div>
          )}
        </div>
...
function RegisterButton({ tournament, allParticipants }: { tournament: Tournament; allParticipants: TournamentParticipant[] }) {
  ...
  if (!user) {
    return (<Button asChild className="bg-gradient-brand"><Link to="/auth"><UserPlus className="mr-2 h-4 w-4" /> Sign in to join</Link></Button>);
  }
  if (existing?.status === "approved") {
    return (<Badge className="bg-emerald-500/20 px-4 py-2 text-emerald-300"><CheckCircle2 className="mr-1.5 h-4 w-4" /> You're registered</Badge>);
  }
  if (existing?.status === "pending") {
    return (<Badge className="bg-amber-500/20 px-4 py-2 text-amber-300"><Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> Join request pending</Badge>);
  }
  if (existing?.status === "rejected") {
    return (<Badge variant="outline" className="px-4 py-2 text-destructive border-destructive/40">Join request rejected</Badge>);
  }
  ...
  return (
    <Button onClick={requestToJoin} disabled={busy || !tournament.registration_open} className="bg-gradient-brand">
...
function ReportForm({ tournament, players }: { tournament: Tournament; players: TournamentParticipant[] }) {
  ...
  const [myReports, setMyReports] = useState<{ id: string; reason: string; description: string | null; player_name: string | null; status: string; created_at: string; resolved_at: string | null; screenshot_url: string | string[] | null }[]>([]);
  ...
  const loadMyReports = useCallback(async () => {
    ...
    const { data, error } = await supabase.from("reports")
      ...
      .eq("reporter_id", user.id).eq("tournament_id", tournament.id)
  ...
  useEffect(() => { void loadMyReports(); }, [loadMyReports]);
  useEffect(() => {
  ...
  if (!user) {
    return (
      <div className="py-6 text-center">
        ...
        <Button asChild className="mt-4 bg-gradient-brand"><Link to="/auth">Sign in</Link></Button>
      </div>
    );
  }
  ...
  if (!isParticipant) {
    return (
      <div className="py-6 text-center">
        <ShieldAlert className="mx-auto h-8 w-8 text-muted-foreground" />
        <p className="mt-3 text-sm text-muted-foreground">Only players registered in this tournament can submit a report.</p>
      </div>
    );
  }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      if (!file.type.startsWith("image/")) { toast.error(file.name + " isn't an image, skipped."); continue; }
    ...
    setScreenshots((prev) => {
      ...
      if (prev.length + validFiles.length > MAX_SCREENSHOTS) toast.error("You can attach up to " + MAX_SCREENSHOTS + " screenshots.");
      return next;
    });
  };

  const removeScreenshot = (index: number) => setScreenshots((prev) => prev.filter((_, i) => i !== index));

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    const trimmedReason = reason.trim();
    if (trimmedReason.length < 4) { toast.error("Please describe the reason for the report."); return; }
    setBusy(true);
    try {
      const urls: string[] = [];
      for (const file of screenshots) urls.push(await uploadPublicImage(file, "reports"));
      const { error } = await supabase.from("reports").insert({
        reporter_id: user.id, type: "tournament", tournament_id: tournament.id,
        player_name: player.trim() || null, reason: trimmedReason.slice(0, 200),
        description: details.trim() ? details.trim().slice(0, 2000) : null,
        screenshot_url: urls.length > 0 ? JSON.stringify(urls) : null, status: "pending",
      });
      if (error) throw error;
      toast.success("Report submitted — the admins will review it.");
      setPlayer(""); setReason(""); setDetails(""); setScreenshots([]);
      await loadMyReports();
    } catch (err) {
  ...
  const statusColor = (status: string) =>
    status === "resolved" ? "bg-emerald-500/20 text-emerald-300"
      : status === "dismissed" ? "bg-secondary text-muted-foreground"
        : status === "in_review" ? "bg-brand/25 text-brand-glow"
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Select value={player} onValueChange={setPlayer}>
            ...
            <SelectContent>{players.map((p) => (<SelectItem key={p.id} value={p.player_name}>{p.player_name}</SelectItem>))}</SelectContent>
          </Select>
        </div>
        <div className="space-y-1.5">
          <label className="text-sm text-muted-foreground">Reason</label>
          <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g. Opponent inactive / toxic" maxLength={200} />
        </div>
        <div className="space-y-1.5">
          <label className="text-sm text-muted-foreground">Details</label>
          <Textarea value={details} onChange={(e) => setDetails(e.target.value)} rows={4} placeholder="Explain what happened..." maxLength={2000} />
        </div>
        <div className="space-y-1.5">
          <label className="text-sm text-muted-foreground">Screenshots (optional, max {MAX_SCREENSHOTS})</label>
            ...
            <div className="flex flex-wrap gap-2">
              {previews.map((url, i) => (
                <div key={url} className="relative">
                  <img src={url} alt="" className="h-16 w-16 rounded-lg object-cover border border-border/60" />
                  <button type="button" onClick={() => removeScreenshot(i)} className="absolute -right-1 -top-1 rounded-full bg-destructive p-0.5 text-white"><X className="h-3 w-3" /></button>
                </div>
              ))}
            </div>
          )}
        </div>
        <Button type="submit" disabled={busy} className="bg-gradient-brand">{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Submit report</Button>
      </form>

      <div>
        <h2 className="text-xl font-bold mb-4">Your reports</h2>
        {loadingReports ? (<Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />)
          : myReports.length === 0 ? (<p className="text-sm text-muted-foreground">No reports yet.</p>)
            : (<div className="space-y-3">{myReports.map((r) => (
                <div key={r.id} className="rounded-xl border border-border/60 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <Badge className={cn("capitalize", statusColor(r.status))}>{r.status.replace(/_/g, " ")}</Badge>
                  </div>
                  {r.player_name && <p className="text-xs text-muted-foreground mt-1">vs {r.player_name}</p>}
                  {r.description && <p className="text-xs text-muted-foreground mt-1">{r.description}</p>}
                  {screenshotList(r.screenshot_url).length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {screenshotList(r.screenshot_url).map((u, i) => (
                        <button key={u} type="button" onClick={() => openLightbox(screenshotList(r.screenshot_url), i)} className="overflow-hidden rounded-lg border border-border/60">
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
          <button type="button" onClick={closeLightbox} className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-white/10 text-white"><X className="h-5 w-5" /></button>
          {lightbox.photos.length > 1 && (<button type="button" onClick={prevPhoto} className="absolute left-2 grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white"><ChevronLeft className="h-5 w-5" /></button>)}
          <img src={lightbox.photos[lightbox.index]} alt="" className="max-h-[85vh] max-w-full rounded-lg object-contain" />
          {lightbox.photos.length > 1 && (<button type="button" onClick={nextPhoto} className="absolute right-2 grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white"><ChevronRight className="h-5 w-5" /></button>)}
        </div>
      )}
    </div>
  );
...
function BracketTab({ matches, matchdays, players, bracketType }: {
  matches: Match[]; matchdays: Matchday[]; players: TournamentParticipant[]; bracketType: string;
}) {
  ...
  const visible = koMatches.filter((m) => m.matchday_id ? mdById.get(m.matchday_id)?.is_published === true : false);
  const locked = koMatches.length > 0 && visible.length === 0;
  if (koMatches.length === 0) {
    return <p className="py-10 text-center text-sm text-muted-foreground">No bracket matches yet.</p>;
  }
  return (
      ...
      {locked && (
        <div className="absolute inset-0 z-10 grid place-items-center rounded-xl bg-background/55 backdrop-blur-[2px]">
          <div className="flex flex-col items-center gap-2 px-4 text-center">
            <span className="grid h-10 w-10 place-items-center rounded-full bg-brand-glow/15 ring-1 ring-brand-glow/40"><Lock className="h-4 w-4 text-brand-glow" /></span>
            <p className="text-sm font-semibold">Bracket not published yet</p>
            <p className="text-xs text-muted-foreground">The bracket will appear here once the admin publishes it.</p>
          </div>
        </div>
      )}
    </div>
...
function MyMatches({ tournament, matches, matchdays, allParticipants, players }: {
